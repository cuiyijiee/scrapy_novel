...
class qidian(Spider):
    ...
    def parse(self, response):
        ...
        for female_page_index in range(1, 26):
            list_page_url = 'https://m.qidian.com/majax/rank/yuepiaolist?gender=female&catId=-1&pageNum=' + \
                            str(female_page_index) + '&_csrfToken=' + token
            yield Request(url=list_page_url, callback=self.parseLastUpdatePage, meta={
                "token": token
            })
        ...
        for female_page_index in range(1, 26):
            list_page_url = 'https://m.qidian.com/majax/rank/hotsaleslist?gender=male&catId=-1&pageNum=' + \
                            str(female_page_index) + '&_csrfToken=' + token
            yield Request(url=list_page_url, callback=self.parseLastUpdatePage, meta={
                "token": token
            })

        for female_page_index in range(1, 36):
            list_page_url = 'https://m.qidian.com/majax/rank/hotsaleslist?gender=female&catId=-1&pageNum=' + \
                            str(female_page_index) + '&_csrfToken=' + token
            yield Request(url=list_page_url, callback=self.parseLastUpdatePage, meta={
                "token": token
            })
